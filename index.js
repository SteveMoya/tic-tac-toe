function Node(value) {
  ...
  function renderPlayer() {
    document.querySelector("#player").textContent = `${
      turn === 0 ? "Turno del Jugador" : "Turno de la Computadora"
    }`;
  }
  ...
  function PCPlaysV2() {
    ...
    // Obtener el valor actual de la barra de dificultad
    const difficulty = parseInt(document.getElementById('difficulty-bar').value);
  
    // Crear el árbol de búsqueda con el nivel de dificultad especificado
    const copy = JSON.parse(JSON.stringify(board));
    const root = new Node(copy);
    processNode(root, true, 0, difficulty);
